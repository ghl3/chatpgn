...
test("getPositionsforPlayer w", () => {
  ...
  expect(MoveSelection.getPositionsForPlayer(game, "foobar")).toStrictEqual([
    {
      game: game,
      moveIdx: "0",
      turn: "w",
...
test("getPositionsforPlayer b", () => {
  ...
  expect(MoveSelection.getPositionsForPlayer(game, "baz")).toStrictEqual([
    {
      game: game,
      moveIdx: "1",
      turn: "b",
...
test("getRandomAcceptablePosition moveIdx===2", () => {
  ...
  expect(
    MoveSelection.getRandomAcceptablePosition(
      game,
      "foobar",
      (p) => p.moveIdx === "2"
    )
...
test("getRandomAcceptablePositionFromGames moveIdx===2", () => {
  ...
  expect(
    MoveSelection.getRandomAcceptablePositionFromGames(
      [game],
      "foobar",
      (p) => p.moveIdx === "2"
    )
