...
export const getPositionsForPlayer = (
  game: Game,
  player: string
): GamePosition[] => {
...
export const getRandomAcceptablePosition = (
  game: Game,
  player: string,
  isAcceptablePosition: PositionSelector
): GamePosition | null => {
  // First, we get all positions in the game for the given player
  const positions: GamePosition[] = getPositionsForPlayer(game, player).filter(
