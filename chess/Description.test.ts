...
test("getColorIndependentScore w", () => {
  expect(Description.getColorIndependentScore("w", 2.5)).toStrictEqual(2.5);
});

test("getColorIndependentScore b", () => {
  expect(Description.getColorIndependentScore("b", 2.5)).toStrictEqual(-2.5);
});

test("doesMoveMaintainForcedMateFor yes", () => {
  expect(
    Description.doesMaintainForcedMateFor(
...
test("doesMoveMaintainForcedMateAgainst yes", () => {
  expect(
    Description.doesMaintainForcedMateAgainst(
...
test("doesBlunderForcedMateAgainst yes", () => {
  expect(
    Description.doesBlunderForcedMateAgainst(
      { depth: 0, forced_mate: { for: "OPPONENT", in: 10 } },
      { depth: 0, score: 10 }
    )
  ).toStrictEqual(true);
...
test("doesMaintainAdvantage yes forced mate", () => {
  expect(
    Description.doesMaintainAdvantage(
      "w",
      { depth: 0, forced_mate: { for: "PLAYER", in: 10 } },
      { depth: 0, forced_mate: { for: "PLAYER", in: 5 } }
    )
...
test("doesMaintainDraw yes", () => {
  expect(
    Description.doesMaintainDraw({ depth: 0, score: 0 }, { depth: 0, score: 0 })
  ).toStrictEqual(true);
});

test("doesMaintainDraw false", () => {
  expect(
    Description.doesMaintainDraw(
      { depth: 0, score: -100 },
      { depth: 0, score: 0 }
    )
  ).toStrictEqual(false);
...
test("makeMoveDescription best move forced mate", () => {
  expect(
    Description.makeMoveDescription(
