...
export class Engine {
  ...
  constructor(worker: WebWorker, depth = 10, num_lines = 1, debug = false) {
    ...
    // The best evaluation of the current position
  ...
  _handleEvent = (e: MessageEvent<any>) => {
    ...
    if (parsed.type === "INFO") {
      ...
      if (moveAndEval.move && moveAndEval.evaluation) {
        this.movesAndEvals.push({
          move: moveAndEval.move,
          evaluation: moveAndEval.evaluation,
        });
      } else if (moveAndEval.evaluation) {
        this.evaluation = moveAndEval.evaluation;
      } else {
    ...
    if (
      ...
      const positionEvaluation = Engine.makePositionEvaluation(
        color,
        fen,
        this.evaluation,
        this.movesAndEvals
  ...
  static buildMoveAndEvaluationFromInfo = (
    ...
    if ("pv" in info && info["pv"].length > 0) {
      ...
      const move: Move = { from: from, to: to, color: color };
      ...
      return { evaluation: Engine.buildEvaluationFromInfo(info, color) };
    } else {
      return {};
    }
  };
  ...
  static makePositionEvaluation = (
    ...
    const bestMoves = Engine.selectBestMoves(color, movesAndEvals);
